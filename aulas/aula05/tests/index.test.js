...
test("se a e b < 0 ou a e b > 0 entao a * b > 0", () => {
    expect(calculadora.multiplicao).toBeDefined();
    expect(calculadora.multiplicao(2, 2)).toBeGreaterThan(0);
    expect(calculadora.multiplicao(-2, -2)).toBeGreaterThan(0);
});

test("se a < 0 ou b < 0 entao a * b < 0", () => {
    expect(calculadora.multiplicao(-2, 2)).toBeLessThan(0);
    expect(calculadora.multiplicao(2, -2)).toBeLessThan(0);
});

test("se a = 0 ou b = 0 entao a * b = 0", () => {
    expect(calculadora.multiplicao(2, 0)).toBe(0);
    expect(calculadora.multiplicao(-2, 0)).toBe(-0);
});
